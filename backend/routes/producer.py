...
async def maintain_memory(request: dict):
    async def change(character, tasks):
        if not character.get("gm_mode", False):
            raise HTTPException(403, "需要高权限模式")
    ...
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "maintain_memory", **payload}, change)
...
async def restore(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "restore", **payload}, change)
...
async def teleport(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "teleport", **payload}, change)
...
async def set_relationship(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "set_relationship", **payload}, change)
...
async def upsert_memory(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "upsert_memory", **payload}, change)
...
async def delete_memory(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "delete_memory", **payload}, change)
...
async def compress_memory(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "compress_memory", **payload}, change)


@router.get("/producer_console/memory/archive")
async def memory_archive(character_id: str, npc_name: str, offset: int = 0, limit: int = 30):
    character = _character(character_id)
    archive = character.get("npc_memory_archive", {}).get(npc_name, [])
    offset, limit = max(0, offset), max(1, min(100, limit))
    return {"total": len(archive), "items": [
        {**item, "archive_id": memory_runtime.memory_identity(item)} for item in archive[offset:offset + limit]
    ]}


@router.post("/producer_console/memory/restore_archive")
async def restore_memory_archive(request: dict):
    async def change(character, tasks):
        if not character.get("gm_mode"):
            raise HTTPException(403, "需要高权限模式")
        try:
            item = memory_runtime.restore_archived_memory(character, str(request.get("npc_name", "")), str(request.get("archive_id", "")))
        except ValueError as exc:
            raise HTTPException(404, str(exc)) from exc
        return {"status": "ok", "memory": item}
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "restore_memory_archive", **payload}, change)
...
async def set_player_state(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "set_player_state", **payload}, change)
...
async def set_resource(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "set_resource", **payload}, change)
...
async def set_anomaly(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "set_anomaly", **payload}, change)
...
async def create_event(request: dict):
    ...
    payload = {key: value for key, value in request.items() if key != "operation_id"}
    return await execute_character_command(request.get("character_id"), request.get("operation_id"),
        {"command": "create_event", **payload}, change)
