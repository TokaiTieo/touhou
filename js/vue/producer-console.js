...
import EvaluationReview from './evaluation-review.js';
import {
    apiCall,
...
export const ProducerConsole = defineComponent({
    ...
    setup(props) {
        ...
        onMounted(async () => {
            await reload(); await loadContentFiles();
            evaluationReport.value = await apiCall('/ghost/producer_console/evaluation/latest?' + new URLSearchParams({ character_id: state.currentSession.characterId })).catch(() => null);
        });
        return {
