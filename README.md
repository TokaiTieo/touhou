...
- V1-V10 旧存档可自动、增量、幂等升级到 V11，并保留未知及自定义字段；人物旧身份自动合并，冲突原始值保留迁移快照。
- 完整剧情分块归档，近期窗口与长期历史分离，支持分页查找、完整导出和独立分支。手动备份时应复制完整数据目录（包含 `_history`），跨机器迁移推荐使用“导出角色”。
...
- 脱敏诊断与测试分发包不包含 API Key、剧情原文或玩家存档；主动导出的反馈包包含角色快照和剧情记录，分享前应确认内容。
